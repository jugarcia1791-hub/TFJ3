document.addEventListener("DOMContentLoaded", function () {
  ...
  const empresaCoords = [40.416775, -3.70379];
  ...
  function mostrarRuta(origenLatLng) {
    ...
    if (
      !origenLatLng ||
      origenLatLng.length !== 2 ||
      isNaN(origenLatLng[0]) ||
      isNaN(origenLatLng[1])
    ) {
    ...
    try {
      ...
      controlRuta.on("routingerror", function (e) {
        ...
        if (infoRuta) {
          infoRuta.innerHTML =
            "❌ No se pudo calcular la ruta. Verifica tu conexión.";
          infoRuta.classList.add("visible");
        }
      });
    } catch (e) {
  ...
  if (navigator.geolocation) {
      ...
      function (error) {
        ...
        switch (error.code) {
          case error.PERMISSION_DENIED:
            mensaje =
              "❌ Permiso denegado. Activa la ubicación y recarga la página.";
            break;
    ...
    if (infoRuta) {
      infoRuta.innerHTML =
        "❌ Tu navegador no soporta geolocalización. Introduce una dirección manualmente.";
      infoRuta.classList.add("visible");
    }
  }
      ...
      fetch(url)
        ...
        .then((data) => {
          if (data && data.length > 0) {
            ...
            if (infoRuta) {
              infoRuta.innerHTML =
                "❌ No se encontró la dirección. Intenta con más detalles.";
              infoRuta.classList.add("visible");
            }
          }
        })
        .catch((error) => {
          ...
          if (infoRuta) {
            infoRuta.innerHTML =
              "❌ Error en la búsqueda. Verifica tu conexión.";
            infoRuta.classList.add("visible");
          }
        });
    });
